...
description: Пошаговое руководство по созданию кросс-платформенного UI-компонента в packages/ui. Используйте при создании atoms, molecules (в components/) или layouts (в layouts/) с реализацией для web и mobile.
---
...
- Имя компонента (например, `Input`, `SearchBar`, `PageLayout`)
- Тип: `atoms`, `molecules` (в `components/`) или `layouts` (в `layouts/`)
...
Путь зависит от типа компонента:

- `atoms`, `molecules` → `packages/ui/src/components/{type}/{ComponentName}/`
- `layouts` → `packages/ui/src/layouts/{ComponentName}/`
...
- `atoms`, `molecules` → `packages/ui/src/components/{type}/`
