...
**Mobile:** `apps/mobile/index.ts` MUST оставаться корневой точкой входа Expo без изменения роли. Корневой UI-кomponent MUST находиться в `apps/mobile/src/app/App.tsx`. Import в `apps/mobile/index.ts` MUST обновиться на `./src/app/App`.
...
- **AND** `apps/mobile/src/app/App.tsx` MUST быть корневым UI-кomponentом
...
- **WHEN** `apps/web` или `apps/mobile` импортирует UI-кomponent
